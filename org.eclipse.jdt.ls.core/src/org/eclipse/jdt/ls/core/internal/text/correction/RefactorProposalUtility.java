...
public class RefactorProposalUtility {
	...
	private static String proposeMethodNameHeuristic(IInvocationContext context, ASTNode coveringNode) {
		...
		Optional<IProblem> res = Arrays.asList(cu.getProblems()).stream()
				.filter(p -> p.getID() == IProblem.LocalVariableIsNeverUsed)
				.filter(p -> p.getSourceStart() >= selStart && p.getSourceEnd() <= selEnd)
		...
		if (res.isPresent()) {
			...
			if (nodeVar instanceof SimpleName) {
				return "get" + ((SimpleName) nodeVar).getIdentifier();
			}
		}
